...
function Flightplan() {
  ...
  self.interruptListener = function () {
    throw new errors.ProcessInterruptedError('Flightplan was interrupted');
  };
  self.exceptionListener = function (err) {
    self.logError(err instanceof errors.BaseError ? err.message : err.stack);
    self.errorCallback(err);
  };

  this.runtime = Object.freeze({});

  process.on('SIGINT', self.interruptListener);
...
Flightplan.prototype.run = function (task, target, options) {
  ...
  if (flights.length === 0) {
    this.logWarning(format("There is no work to be done for task '%s'", task));
  ...
  this.logInfo(format('Running %s:%s', task, target));
  ...
  new Fiber(function () {
    if (config.hosts) {
      ...
      if (typeof config.hosts === 'function') {
        ...
        self.logInfo('Running dynamic hosts configuration');
    ...
    try {
      ...
      self.logInfo('Flightplan finished after ' + prettyTime(process.hrtime(t)));
    } catch (e) {
      self.errorCallback(e);
      self.logError('Flightplan failed after ' + prettyTime(process.hrtime(t)));
    } finally {
      flight.disconnect();
      process.removeListener('SIGINT', self.interruptListener);
...
Flightplan.prototype.addLogger = function (newLogger) {
  this._loggers.push(newLogger);
};
...
Flightplan.prototype.logError = function (msg) {
  this._loggers.forEach(function (l) {
    l.error(msg);
  });
};

Flightplan.prototype.logWarning = function (msg) {
  this._loggers.forEach(function (l) {
    l.warn(msg);
  });
};

Flightplan.prototype.logInfo = function (msg) {
  this._loggers.forEach(function (l) {
    l.info(msg);
  });
};
